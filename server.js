...
let partidas = [
  { id: 1, time_casa: 'Ceará', time_visitante: 'Fortaleza', status: 'Em andamento', gols_casa: 1, gols_visitante: 1, competicao: 'Campeonato Cearense' },
  { id: 2, time_casa: 'Flamengo', time_visitante: 'Palmeiras', status: 'Não iniciada', gols_casa: 0, gols_visitante: 0, competicao: 'Brasileirão' },
  { id: 3, time_casa: 'Real Madrid', time_visitante: 'Barcelona', status: 'Encerrado', gols_casa: 2, gols_visitante: 1, competicao: 'La Liga' }
];

let gols = [
  { id: 1, partida_id: 1, minuto: 15, jogador: 'Vina', tipo: 'Normal', tempo_etapa: '1º Tempo', assistencia: 'Erick' },
  { id: 2, partida_id: 1, minuto: 42, jogador: 'Lucero', tipo: 'De Pênalti', tempo_etapa: '1º Tempo', assistencia: 'Nenhuma' },
  { id: 3, partida_id: 3, minuto: 89, jogador: 'Vinicius Jr', tipo: 'Normal', tempo_etapa: '2º Tempo', assistencia: 'Bellingham' }
];
...
app.get('/partidas/:busca', (req, res, next) => {
  ...
  const filtradas = partidas.filter(p => p.time_casa.toLowerCase() === busca.toLowerCase());
  
  if (filtradas.length === 0) {
    return res.status(404).json({ erro: 'Nenhuma partida encontrada para este mandante' });
  }
